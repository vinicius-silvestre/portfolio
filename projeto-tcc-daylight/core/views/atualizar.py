...
def update_ordem_servico(request, id):
    update_ordem_servico = Ordem.objects.get(id=id)
    form = GerenciarForm(request.POST or None, instance=update_ordem_servico)

    if form.is_valid():
        form.save()
        return redirect('list_ordem_servico')

    return render(request, 'cadastro/ordemservico.html', { 'form' : form, 'update_ordem_servico':update_ordem_servico })
